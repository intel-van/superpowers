...
Claude Code 插件需要能在 Windows、macOS 和 Linux 上运行的钩子。本文档说明了实现这一目标的多语言包装器技术。
...
多语言脚本同时是多种语言的有效语法。我们的包装器在 CMD 和 bash 中都有效：
...
1. `: << 'CMDBLOCK'` — `:` 是无操作指令，`<< 'CMDBLOCK'` 开始一个 heredoc
...
实际的钩子逻辑放在 `.sh` 文件中。为确保其在 Windows（通过 Git Bash）上正常工作：
...
- 如果必须使用它们，Git Bash 中确实可用，但要确保 PATH 已设置（使用 `bash -l`）

### 示例：无需 sed/awk 的 JSON 转义
...
对于包含多个钩子的插件，可以创建一个通用包装器，将脚本名称作为参数传入：
...
Bash 未以登录 shell 方式运行。确保使用了 `-l` 标志。
...
- [anthropics/claude-code#9758](https://github.com/anthropics/claude-code/issues/9758) — .sh 脚本在 Windows 上会在编辑器中打开
- [anthropics/claude-code#3417](https://github.com/anthropics/claude-code/issues/3417) — 钩子在 Windows 上无法工作
- [anthropics/claude-code#6023](https://github.com/anthropics/claude-code/issues/6023) — 找不到 CLAUDE_PROJECT_DIR

---

[← 返回主 README](../../../README.md) · [English Version](../../windows/polyglot-hooks.md)
