...
**目标：** 将 brainstorm 服务器的 vendored node_modules 替换为单个零依赖的 `server.js`，使用 Node 内置模块。

**架构：** 单文件包含 WebSocket 协议（RFC 6455 文本帧）、HTTP 服务器（`http` 模块）和文件监视（`fs.watch`）。在作为模块被 require 时导出协议函数以进行单元测试。
...
服务器帧从不掩码。三种长度编码：
- payload < 126：2 字节头部（FIN+opcode，长度）
- 126-65535：4 字节头部（FIN+opcode，126，16 位长度）
- > 65535：10 字节头部（FIN+opcode，127，64 位长度）
...
客户端帧总是掩码的。返回 `{ opcode, payload, bytesConsumed }` 或不完整时返回 `null`。未掩码帧抛出异常。
...
在模块作用域加载 `frameTemplate` 和 `helperInjection`，以便 `wrapInFrame` 和 `handleRequest` 可以访问它们。它们只从 `__dirname`（脚本目录）读取文件，无论模块是被 require 还是直接运行都有效。
...
文件监视逻辑内联在 `startServer` 中（步骤 6），以保持监视器生命周期与服务器生命周期一致，并按规范包含 `error` 处理器。

- [ ] **步骤 6：添加 startServer 函数和条件主入口**

`frameTemplate` 和 `helperInjection` 已在模块作用域（步骤 2）中。`startServer` 仅创建屏幕目录、启动 HTTP 服务器、监视器并记录启动信息。
...
测试目录已经有一个包含 `ws` 依赖的 `package.json`。如有需要安装它，然后运行测试。
...
- [ ] **步骤 2：打开浏览器访问 http://localhost:9876**

预期：显示 "Waiting for Claude to push a screen..." 等待页面
...
预期：浏览器重新加载并显示包裹在框架模板中的 "Hello from smoke test"

- [ ] **步骤 4：验证 WebSocket 正常工作 — 检查浏览器控制台**
...
[← 返回主 README](../../../../README.md) · [English Version](../../../superpowers/plans/2026-03-11-zero-dep-brainstorm-server.md)
